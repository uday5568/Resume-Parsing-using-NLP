...
class MainWindow():
    ...
    def login_gui(self):
        ...
        self.gui_elements.append(Label(self.root, text="Username", font=('Arial', 16), width=10))
        ...
        self.gui_elements.append(Label(self.root, text="Password", font=('Arial', 16), width=10))
        ...
        self.gui_elements.append(Button(self.root, text="Create Account", font=('Arial 12'), fg="#00f", bg="#012", bd="0",
               command=self.signup_gui,
               width=12, height=1))
        self.gui_elements[-1].place(x=login_x + 245, y=login_y + 87)

        self.gui_elements.append(Button(self.root, text="Login", font=('Arial 12'), command=self.back_to_main, width=10, height=1))
    ...
    def signup_gui(self):
        ...
        def fun():
            ...
            try:
                ...
                db.execute(sql)
                if str(hashlib.md5(username_s.get().encode()).hexdigest()) in [i[0] for i in db]:
                    errmsg = Label(self.root, text="*Username Taken", font=('Arial', 12), fg='#ff0000', bg='#012', width=20)
                    errmsg.place(x=700, y=350)
                    self.root.after(3000, errmsg.destroy)
                else:
                    sql = "INSERT INTO users VALUES(MD5(%s),MD5(%s))"
                    ...
                    if re.search('[A-Z]', val[1]) and re.search('[a-z]', val[1]) and re.search('[0-9]',
                            ...
                            '[@_!#$%^&*()<>?/\|}{~:]', val[1]) and len(val[1]) >= 8:
                        db.execute(sql, val)
                        errmsg = Label(self.root, text="*Registration Successful", font=('Arial', 12), fg='#00ff00',
                                       bg='#012', width=30)
                        errmsg.place(x=700, y=350)
                        self.root.after(3000, errmsg.destroy)

                    else:
                        errmsg = Label(self.root, text="*Weak Password Try Again", font=('Arial', 12), fg='#ff0000',
                                       bg='#012', width=30)
                        errmsg.place(x=700, y=350)
                        self.root.after(3000, errmsg.destroy)

            except Exception as e:
                errmsg = Label(self.root, text="*Registration Failed, Error Occured", font=('Arial', 12), fg='#ff0000',
                               bg='#012', width=30)
                errmsg.place(x=700, y=430)
                self.root.after(3000, errmsg.destroy)
        ...
        self.gui_elements[-1].place(x=signup_x + 200,y=signup_y + 130)
        self.root.mainloop()
